...
def read_ravdess_video(par_path, with_audio = True, mode="av"):
    if mode not in ["av", "video", "audio"]:
        raise ValueError(f"Unsupported mode: {mode}. Choose 'video' or 'audio'.")
    # emotion map
    classes_map = {"03":"happiness", "04":"sadness", "01": "neutral", "05": "anger", "08":"surprise", "07": "disgust", "06":"fear"}

    # find all video files within the dataset
    if with_audio:
        all_videos = glob.glob(f"{par_path}/ravdess_videos_24fps/*/01-*.mp4")
    else:
        all_videos = glob.glob(f"{par_path}/ravdess_videos_24fps/*/02-*.mp4")
    all_videos.sort()
    all_labels = []
    all_fpaths = []
    for fpath in all_videos:
        ...
        try:
            emotion = classes_map[fname.split("-")[2]]
        except:
            # skip cases where emotion is not amongst the interested 7.
            continue
        if with_audio:
            audio_fpath = fpath.replace(".mp4", ".wav")
            audio_fpath = audio_fpath.replace("ravdess_videos_24fps", "ravdess_videos_16khz")
        ...
        if mode == "av":
            ...
            all_fpaths.append({"video": fpath})
        elif mode == "audio":
            all_fpaths.append({"audio": audio_fpath})
        else:
            raise ValueError(f"Unsupported mode: {mode}. Choose 'video' or 'audio'.")
...
class EvaluateRAVDESSVideo:

    def __init__(self, with_audio = True, eval_prompt_type = "single_label", mode="av"):
        dataset_par_path = os.path.join(VIDEO_EVAL_DATA_PAR, "RAVDESS")
        self.file_paths, self.labels = read_ravdess_video(dataset_par_path, with_audio = with_audio, mode=mode)
    ...
    def evaluate(self, pred_path):
        ...
        for fpath_dict, label in zip(self.file_paths, self.labels):
            fpath = fpath_dict["video"] if "video" in fpath_dict else fpath_dict["audio"]
            fname = fpath.split("/")[-1].split(".")[0]
