...
from videollava.mm_utils import tokenizer_image_token, tokenizer_audio_token, tokenizer_audio_and_image_token
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            ...
            if 'audio' not in sources[0] and 'image' in sources[0] and 'video' not in sources[0]:
                # rank0_print('image')
            ...
            elif "audio" in sources[0] and 'image' not in sources[0] and 'video' not in sources[0]:
                ## Only audio
                if "processed_audio" not in sources[0]:
                    ...
                    # print("AUDIO ==>", audio)
                    audio = [speech_processor(aud, return_tensors="pt")["spectrogram"][0] for aud in audio]
                else:
                    aud = np.load(os.path.join(self.data_args.audio_folder, sources[0]['processed_audio']))
                    audio = [torch.tensor(aud)]
                
                # print("The code is not reaching here = 2.5")
                sources = preprocess_multimodal(copy.deepcopy([e["conversations"] for e in sources]), self.data_args)
                data_dict = preprocess(sources, self.tokenizer, has_image=False, has_audio=True)
                # print("Data_dict after preprocess()", data_dict)
                # print("The code is not reaching here = 5")
            elif 'audio' not in sources[0] and 'image' not in sources[0] and 'video' in sources[0]:
                # rank0_print('video')
                # only video
                if "processed_video" not in sources[0]:
                    ...
                    print(f"Loading processed video from {os.path.join(self.data_args.video_folder, sources[0]['processed_video'])}")
                    video = np.load(os.path.join(self.data_args.video_folder, sources[0]['processed_video']))
            ...
            elif 'audio' in sources[0] and 'image' not in sources[0] and 'video' in sources[0]:
                ...
                if "processed_audio" not in sources[0]:
                    ...
                    aud = np.load(os.path.join(self.data_args.video_folder, sources[0]['processed_audio']))
                    audio = [torch.tensor(aud)]
                ...
                ## print shapes of video after extracting frames
                # print(f"Video file shape after extracting frames -->", [video_file.shape for video_file in image]) ## (8,3,224,224) for each video file

                # now we combine the audio and video
                # first check if the text of conversation contains only video or audio tag. 
                # if only video tag is present, add <audio> tag to the conversation as well # FIXME
                sources = preprocess_multimodal(copy.deepcopy([e["conversations"] for e in sources]), self.data_args)
                # print('after preprocess_multimodal', sources[0])
                data_dict = preprocess(sources, self.tokenizer, has_image=True, has_audio=True)
                # print('after preprocess', data_dict['input_ids'])

            elif 'audio' not in sources[0] and 'image' in sources[0] and 'video' in sources[0]:
                ...
                print("NO IMAGE OR VIDEO OR AUDIO FOUND.......")
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        for instance in instances:
            ...
            if "audio" in instance:
                audio_present = True
        # print(f"image_present: {image_present}, audio_present: {audio_present}")
        if image_present:
        ...
        if audio_present:
            ...
            audios = new_audios
        # ==========Too many videos or audios may lead to OOM, so we encode them one by one======================
            batch['audios'] = audios
        if not image_present and not audio_present:
            # if no image or audio, then we return the batch
