...
class IEMOCAP(RawAudioDataset):
    def __init__(self, dataset_name, dataset_path):
        ...
        self.classes = ["neu", "hap", "ang", "sad"]
        self.classes_map = {"hap":"happiness", "ang":"anger", "sad":"sadness", "neu":"neutral"}

        ## read label metadata for all sessions
        all_json_files = glob.glob(os.path.join(self.dataset_path, "meta_data/Session*/*.json"))
        self.label_dict = {}
        for jsonf in all_json_files:
            with open(jsonf) as f:
                cur_metadata = json.load(f)["meta_data"]
            for md in cur_metadata:
                fpath = md["path"]
                emotion_gt = self.classes_map[md["label"]]
                self.label_dict[fpath] = emotion_gt
        print(list(self.label_dict.keys())[:10])
    ...
    def convert_to_16khz(self, dataset_path):
        ...
        all_relevant_audios = glob.glob(f"{dataset_path}/Session*/sentences/wav/*/*.wav")
        all_files = []
    ...
    def get_label_from_fname(self, fpath):
        rel_fpath = "/".join(fpath.split("/")[-self.wav_depth:])
