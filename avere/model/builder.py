...
from avere.constants import DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, \
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", **kwargs):
    ...
    if 'llava' in model_name.lower() or 'avere' in model_name.lower():
        # Load LLaVA model
        ...
        if 'lora' in model_name.lower() and model_base is not None:
            ...
            mm_projector_exists = os.path.exists(os.path.join(model_path, 'mm_projector.bin'))
            speech_projector_exists = os.path.exists(os.path.join(model_path, 'speech_projector.bin'))
            if mm_projector_exists and speech_projector_exists:
                raise NotImplementedError("You have passed a model path with both speech and mm projector weights. Please check what you are doing...")
            if mm_projector_exists and not speech_projector_exists:
                print("LOADING MM PROJECTOR WEIGHTS FROM ===>", os.path.join(model_path, 'mm_projector.bin'))
                mm_projector_weights = torch.load(os.path.join(model_path, 'mm_projector.bin'), map_location='cpu')
                mm_projector_weights = {k: v.to(torch.float16) for k, v in mm_projector_weights.items()}
                model.load_state_dict(mm_projector_weights, strict=False)
            if not mm_projector_exists and speech_projector_exists:
                print("LOADING SPEECH PROJECTOR WEIGHTS FROM ===>", os.path.join(model_path, 'speech_projector.bin'))
